...
class BasketController extends Controller {
    public function index(Request $request) {
        ...
        if (!empty($basket_id)) {
            $posts = Basket::findOrFail($basket_id)->posts;
            return view('basket.index', compact('posts'));
        } else {
            abort(404);
        }
    }
    ...
    public function add(Request $request, $id) {
        ...
        if ($basket->posts->contains($id)) {
            // если такой товар есть в корзине — изменяем кол-во
            $pivotRow = $basket->posts()->where('post_id', $id)->first()->pivot;
            $quantity = $pivotRow->quantity + $quantity;
            $pivotRow->update(['quantity' => $quantity]);
        } else {
            // если такого товара нет в корзине — добавляем его
            $basket->posts()->attach($id, ['quantity' => $quantity]);
        }
